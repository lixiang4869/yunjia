  ...
  <view class="flex-column">
    ...
    <view>
      <view class="infoContentTitle">
        <image class="infoPic" src="https://xin.yiyunoto.com/uploads/static/info-circle.png"/>
    ...
    <view v-if="!currentIsLatest">
      <view class="infoContentTitle">
        <image class="infoPic" src="https://xin.yiyunoto.com/uploads/static/sync.png"/>
    ...
    <view class="minorContent bottom_aera">
      ...
      <button v-if="!startProgress && !currentIsLatest"  class="update" @click="handleUpdate()">立即更新</button>
      <button v-if="currentIsLatest" :loading="buttonLoading"  class="update" @click="getLatest()">检查更新</button>
    </view>
  </view>
</template>
...
export default {
  ...
  methods: {
    ...
    getLatest() {
      ...
      uni.request({
        url: this.url+'/api/index/getAppConfig',
        method: 'GET',
    ...
    // 检查版本
    checkLatest() {
      if (!this.latest.newversion) {
        this.Mtip = '未找到发行版本'
      } else if (this.$current.id < this.latest.newversion) { // 当前版本与新版本不符（$current在main.js里）
        this.currentIsLatest = false
        this.Mtip = '发现新版本'
		var baiwei=parseInt(this.latest.newversion/100);
		  var shiwei=parseInt((this.latest.newversion%100)/10); 
		      var gewei=parseInt(this.latest.newversion%10);  
        this.info = baiwei + '.'+shiwei+'.'+gewei //名称 版本号
        this.updateInfo = this.latest.content
        this.packgeSize = (this.latest.packagesize / 1024).toFixed(2) + 'MB' // 请求取得的packgeSize是字节大小，转换为保留两位小数的MB
		this.Minfo = '正式版'
    ...
    handleUpdate() {
      ...
      if (plus.os.name.toLowerCase() === 'android') {
        ...
        if (this.latest.pkgurl && this.latest.pkgurl !== '#') { // 我这里默认#也是没有地址，请根据业务自行修改
          // 安卓：创建下载任务
          this.createTask(this.url+this.latest.pkgurl)
        } else {
