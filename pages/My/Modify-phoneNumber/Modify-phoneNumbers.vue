<template>
	...
	<view>
		<view class="form re">
			<view class="username"><input placeholder="请输入绑定的手机号" maxlength="11" v-model="phoneNumber" placeholder-style="color:rgba(205,205,205,1);font-size:26upx;" /></view>
			<view class="code">
			...
			<view class="btn" @tap="amendPhone">绑定</view>
		</view>
	</view>
</template>
			...
			var authtoken = uni.getStorageSync("authtoken")
				request.urlRequest(
					...
					res => {
						...
						if (res.data.code == 1) {
							let infor = this.$store.state.userInfor;
							infor.meminfo.phone=this.phoneNumber;
							uni.setStorageSync("phone", this.phoneNumber);
							this.$store.commit("changeUserInfor", infor);
							uni.showToast({
								title:'绑定成功',
								icon: 'none'
							});
