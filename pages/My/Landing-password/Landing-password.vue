<template>
	...
	<view>
		<view class="form re">
			...
			<view class="password">
				<input placeholder="请设置新的登录密码" type="text"  :password="true"  v-model="passwd"   placeholder-style="color:rgba(205,205,205,1);font-size:26upx; "/>
			</view>
...
export default {
	...
	methods: {
		...
		// 修改登录密码
		queren() {
			console.log(this.phoneNumber,this.code,this.code)
			let authtoken = uni.getStorageSync("authtoken");
			request.urlRequest(
				this.url + '/api/member/editpwd',
				{
					phone: this.phoneNumber,
					code: this.code,
					pwd: this.passwd,
