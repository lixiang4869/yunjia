<template>
	<view class="quan">
		<!-- 头部 -->
			...
			<view class="top" @click="goToday">
				<view class="top1">
					<text class="t1">今日收款{{count}}笔</text>
					<view >
	...
	export default {
		...
		methods: {
			...
			getdata(){
				...
				request.urlRequest(this.url+"/api/Shopinfo/shopinfo",{},{authtoken: authtoken},"POST",res=>{
					if(res.data.code==1){
						this.money = res.data.data.money;
						this.count = res.data.data.order_count;
						this.today =  res.data.data.total_amount;
					}else{
						uni.showToast({
