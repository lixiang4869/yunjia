	...
	<view
		...
		<view class="message" v-for="item in chatMsg" :key="item.mid" :id="item.mid">
			...
			<view class="main" :class="item.style">
				<view class="user">
					<!-- yourname：就是消息的 from -->
					<text class="user-text">{{ (item.style == 'self'?item.yourname:"客服")+ ' ' + item.time}}</text>
				</view>
				
				<image class="avatar" v-if="item.style == 'self'" :src="myHead"/>
				<image class="avatar"  src="/static/video/me.png" v-else/>
				<view class="msg">
					<image
